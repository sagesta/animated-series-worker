...
- Preserved the first terminal controller record so the detached deadline monitor cannot overwrite an earlier explicit operator termination reason after the provider already deleted the Pod.
...
- Added D-053 and a manual-only/main-only `sign-worker-image.yml` canonical signer. It fixes the GHCR package, requires strict manifest/config digests, uses commit-pinned Cosign/login actions, requests only contents-read/OIDC permissions, takes registry transport authentication from the protected `GHCR_SIGNING_TOKEN` environment secret, binds the `worker-signing` environment, and verifies the exact GitHub workflow identity after signing. Four local policy tests pass.
- Published and executed the protected canonical signer from `main` in [GitHub Actions run 32967547472](https://github.com/sagesta/animated-series-worker/actions/runs/32967547472). The run validated the exact manifest/config pair, signed through GitHub OIDC, and self-verified the exact workflow identity. Independent Cosign verification passed, and the canonical signature is stored as OCI referrer `sha256:c979edccb17c97a217b015429d9e39b831259389b5fdf3a69b9dc7c81e81b094` with child manifest `sha256:bc8b48f0c560a66fb7f36d5e10e9c0bf44a442d84dd964e097f918bf82c3291b`. The temporary registry token secret was deleted immediately after the successful run.
- Kept the image undeployed and release-locked. Canonical signing does not provide model/license approval, model-backed GPU qualification, media-quality proof, provider lifecycle/cost evidence, or a production-readiness receipt, and no RunPod resource was created.
