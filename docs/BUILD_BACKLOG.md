...
The current source provides a working local Windows shell, isolated series/film projects, immutable Audience & Creative Direction revisions, guided required-field/length/range warnings, verified backup/non-overwriting restore, single-writer protection, protected RunPod/OpenAI/Anthropic/Gemini keys, read-only RunPod checks, a controlled stable writing-model catalogue, and provider-neutral local writing proposals with exact direction lineage. It cannot yet provision a GPU, run ComfyUI, generate media, execute an external creative skill, promote proposals into versioned canon, or assemble an episode.
