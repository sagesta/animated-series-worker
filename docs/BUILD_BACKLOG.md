...
The current source provides a working local Windows shell, isolated series/film projects, immutable Audience & Creative Direction revisions, guided required-field/length/range warnings, verified backup/non-overwriting restore, single-writer protection, protected RunPod/OpenAI/Anthropic/Gemini keys, read-only RunPod checks, a controlled stable writing-model catalogue, provider-neutral local writing proposals with exact direction lineage, and project-scoped declarative creative skills with visible plans and receipts. It cannot yet provision a GPU, run ComfyUI, generate media, execute higher-risk tool/MCP skills, promote proposals into versioned canon, or assemble an episode.

## 5. Master work stack

| Order | Work package | Phase | Status | Required outcome | Exit proof |
| --- | --- | --- | --- | --- | --- |
| 1 | FOUND-001 desktop foundation completion | 1 | In progress — shared required markers, live limits, accessible correction popups, and safe no-silent-disable actions pass renderer tests | Stable packaged shell, structured safe errors, support diagnostics, and responsive non-technical UI | Clean-machine launch, automated accessibility, interruption, representative-user AT-060, and signed-package evidence |
...
| 5 | SKILL-001 external creative-skill runtime | 2 | In progress — safe declarative JSON install/inspect, project grants, task routing, compatibility/permission blocking, stale-plan refusal, required section validation, immutable receipts, update grant revocation, removal preservation, and Settings/Creative Room UI pass locally | Complete signed-package/update comparison, broader schema validators, future tool/MCP isolation/timeouts, packaged security scan, and non-technical acceptance | AT-037 plus extension security suite pass |
