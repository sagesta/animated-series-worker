...
| WORKER-001 | Implemented, signed, and live technically exercised | Candidate 5 mapped models into ComfyUI, supported proxy-safe verified downloads, handed VRAM from ComfyUI to TTS, proved 11 hashes/seven workflow executions on an L40S, and was canonically signed by exact digest | Correct failed creative/duration acceptance and finish durable-storage/recovery/shutdown/cost evidence |
...
| BENCH-002 | Release-blocked by observed output | Exact live artifacts preserve the failed blue-scarf edit and the 0.75-second final-video result | Correct workflow contracts, rerun the same fixtures, and obtain explicit human image/audio/video approval |
...
| COMFY-001 | Implemented and live technically exercised | Candidate registry, exact graphs, allowlists, and core model paths queued successfully on the L40S | Correct and rerun failed edit/duration acceptance |
...
| BENCH-001 | Implemented with partial live evidence | Seven core workflows technically succeeded on the exact image/GPU | Fix observed quality/contract failures and complete security/recovery/shutdown/cost fixtures |
| MEDIA-001 | Implemented candidate with live outputs | Qwen identity/edit and Qwen3-TTS voice/line-book executed with exact hashes | Correct failed blue-scarf edit and complete human image/audio review |
| QWEN-EDIT-001 | Implemented prospective fix; targeted live proof blocked | `qwen-image-targeted-edit@1.0.1` requires ordered parent/mask inputs, mask-binds prompt and inpaint sampling, restores parent pixels outside the mask, clears contradictory clothing-negative conditioning, and rejects the failed `0.3` denoise setting; regression fixture/tests pass locally | Build a new fingerprinted immutable worker layer, run only the blue-scarf fixture when reusable model storage exists, prove in-mask change/outside-mask preservation, and obtain human review |
| VIDEO-001 | Implemented candidate with live core outputs | LTX draft/final executed at 1280×704/12 fps | Correct requested duration, run human quality review, and keep audio-driven/LatentSync separately gated |
