...
- **FR-030:** The scheduler groups compatible jobs to reduce model reloads, selects the exact qualified worker profile required by the workflow, and refuses a GPU or worker image whose verified VRAM/capabilities do not meet that requirement. Optional adaptation cannot route through the normal core-generation worker.
...
- **NFR-018 Immutable production runtime:** A production worker cannot install or update ComfyUI, custom nodes, Python/CUDA dependencies, models, LoRAs, or workflows while executing an authorized session. Changes occur only through a separately built, scanned, benchmarked, pinned, and rollback-capable worker release. Core generation and optional advanced/training profiles may use different immutable images and evidence receipts; an unavailable profile is blocked rather than installed at runtime.
