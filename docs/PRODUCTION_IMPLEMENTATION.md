...
The candidate pack contains reviewed, hash-locked API graphs for Qwen character/storyboard frames, Qwen targeted edits, an LTX single-stage draft, an LTX two-stage final, native LTX audio-driven dialogue, control-guided Qwen, and control-guided LTX. Final I2V workflow `1.0.1` corrects the live duration defect by rounding generation up to LTX's pinned `8n+1` decoded-frame boundary and then strictly slicing the assembled audio/video to the requested duration; the draft workflow and historical `1.0.0` evidence are unchanged. Rights-aware foley and project adaptation have exact hash-locked runner contracts. All are structurally checked and non-billable. Core promotion is atomic across every policy-eligible core and local-finishing entry and includes only the models those workflows reference. `core-promotion-policy.json` restrictively excludes LatentSync lip repair and its model dependencies without changing the already signed candidate worker bytes. Advanced and policy-excluded entries remain candidate-only until a separate profile passes its own declared license, model, GPU runtime, output quality, cost, rights, security, recovery, and shutdown evidence.

The failed August 26 targeted edit is corrected prospectively in `qwen-image-targeted-edit@1.0.1`: the request now requires the parent image followed by a region-mask image; the edit instruction is bound to that mask; the mask drives the inpaint latent; and a final masked composite restores the original-resolution parent outside the editable region. The graph also removes the contradictory `changed clothing` negative conditioning and refuses the failed `0.3` denoise setting in favor of a `0.6` minimum and `1.0` default. Local tests lock the graph, UI ordering, mask fixture, and low-strength refusal. This is not live quality evidence: a newly fingerprinted immutable worker candidate and the targeted blue-scarf benchmark must still pass before the defect can be called technically fixed.
...
- the targeted edit failed its explicit blue-scarf instruction, and the live final video was 0.75 seconds for a one-second request; final I2V `1.0.1` now has a local structural fix and 1/2/4-second regression coverage, but targeted GPU duration evidence and human image/audio/video approval remain open;
