...
Implementation consequence: the studio uses provider-neutral writing tasks with OpenAI, Anthropic, and Gemini adapters rather than making any provider conversation the project database. Version 0.8.0 uses model-list reads for no-cost key validation, intersects availability with the controlled stable catalogue, then uses OpenAI `POST /responses` with `store: false`, Anthropic `POST /v1/messages`, or Gemini `POST /v1beta/models/{model}:generateContent` with structured output. It previews the exact selected manifest and creative-direction context, refuses partial/unreadable structures, records usage/request IDs and source hashes locally, and leaves dollar cost uncalculated until versioned price profiles and live benchmarks exist. The first declarative external-skill slice is a local contract/security implementation: schema-3 proposals record the exact previewed plan and receipts, while general signature verification, tool/MCP execution, and current external interoperability standards remain open selections rather than externally verified claims.
...
| Pod lifecycle/API | RunPod documents create, get/list, start/stop, and permanent delete operations. Version 0.9 implements them behind lease reconciliation, cost/start approval, qualification, and shutdown gates. | [Create Pod](https://docs.runpod.io/api-reference/pods/POST/pods), [Get Pod](https://docs.runpod.io/api-reference/pods/GET/pods/podId), [Delete Pod](https://docs.runpod.io/api-reference/pods/DELETE/pods/podId), [Manage Pods](https://docs.runpod.io/pods/manage-pods) |
