...
Implementation consequence: the studio uses provider-neutral writing tasks with OpenAI, Anthropic, and Gemini adapters rather than making any provider conversation the project database. It uses model-list reads for no-cost key validation, intersects availability with the controlled stable catalogue, then uses OpenAI `POST /responses` with `store: false`, Anthropic `POST /v1/messages`, or Gemini `POST /v1beta/models/{model}:generateContent` with structured output. Version 0.10 reuses this path for its field-level assistant: exact project context and skill plan are previewed, every provider call requires confirmation, output remains a schema-3 proposal, and secrets/evidence/attestations/spend/publishing stay outside its write authority. Dollar cost remains uncalculated until versioned price profiles and live benchmarks exist; general signatures, tool/MCP execution, and current external interoperability remain open selections.
...
| LatentSync lip repair | The official repository/model card provides the 1.6 inference path; the repository reports an 18 GB inference requirement. Code and model licenses are reviewed separately during promotion, and the animated/multi-angle fixture pack remains mandatory. | [LatentSync repository](https://github.com/bytedance/LatentSync), [LatentSync-1.6 model card](https://huggingface.co/ByteDance/LatentSync-1.6) |
...
Design consequence: default video delivery remains a versioned 1080p/24fps SDR H.264/AAC profile with technical QC. Public-facing thumbnails, release metadata, chapters, audience/disclosure attestations, and analytics definitions use versioned rules rechecked before release changes. Version 0.10 stores structured manual official/report/rehearsal snapshots but does not parse a report file or request OAuth. Version 1 exports a verified manual-upload package and does not automatically publish.
