...
| Optional analytics/auto-publishing | Outside version 0.9 | Separately authorized product/security work only |
...
- The complete quality gate passes: documentation, TypeScript, formatting, lint, 98 tests across 22 files, and the production Electron build.
- The 29 focused provider, workflow-registry, worker-client, readiness, production-store, and orchestrator tests are included in that passing suite.
- The corrected isolated unpacked Windows package is at `release-smoke-0.9.0-installer-lean/win-unpacked`. Its executable reports product version `0.9.0.0`, is 235,534,336 bytes, and has SHA-256 `43E4B56DD04BD1F2CE103F671B8B436F2EDF2D52EDE91DCCEBFC541CE1CB0F5D`.
- The NSIS installer is `release-smoke-0.9.0-installer-lean/Animated-Series-Studio-0.9.0-x64.exe`, is 105,030,036 bytes, and has SHA-256 `779868A6F3D68157D33D509E36F31C8B45D27B27E2DECC0994A65FC1B4119D84`.
- A hidden isolated-profile smoke of the corrected package stayed running for eight seconds and reused its valid local project catalogue. The process tree was then stopped. Executable and installer are `NotSigned`, use the default Electron icon, and are development-machine evidence rather than a production release.
