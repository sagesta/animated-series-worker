...
| Project-store integration tests | Twelve passing cases covering series+film create/reopen, guided/verified v1→v2 migration, rollback after four injected failure points, verified full backup/restore with byte fixtures, no-overwrite recovery, damaged-backup refusal, canonical files/databases/folders, same-title isolation, path-like identity rejection, and live/stale/incomplete writer locks | Automated AT-001 and the migration slice of AT-004 pass locally; non-migration interruption, future migration breadth, clean-machine AT-030, and full AT-031 isolation remain |
| Renderer tests | Six passing tests covering honest no-paid-service empty state, four-step one-off-film creation plus the plain-language verified-backup action, migration preview/backup/update, Settings recovery of a missing project, local redacted-support-file creation, and the no-cost RunPod connection flow remaining generation-locked | Does not replace representative non-technical usability acceptance, packaged secret scan, clean-machine recovery drill, or live-provider test |
...
The suite contains 37 automated tests. Automated AT-001 now passes for a series and film on the development machine, and the v1→v2 migration portion of AT-004 passes at four injected failure points; broader non-migration interruption and future-migration evidence plus the clean-machine AT-030 drill remain. AT-013 remains open because no provider resource, worker, job, purge, watchdog, termination, or charge has been tested. AT-031 remains open because asset/query/token/cache and concurrent-worker isolation do not exist yet.
