...
Current implementation note (0.10.1): local configuration/syntax/type/unit/renderer and production-safety suites cover the implemented control plane, five public-domain novel fixtures plus empty/single-character/500k-character boundaries, governed idea-assistant behavior, release planning/report evidence/learning, adaptation dataset integrity, and non-billable advanced workflow definitions. Real Electron tests cover the five-step first-run flow, two disclosure levels at 1280×720, keyboard correction summary, verified backup/restore with approved lineage, cross-project refusal/explicit copy, and hash-inventoried release packaging. `gpu-qualification-evidence.template.json` and the guarded promotion tool make the independently packaged core image/voice/video/security/recovery/shutdown/cost evidence executable as a release gate; advanced and adaptation profiles remain separately locked. A controlled L40S run produced partial live evidence, but the edit/duration acceptance and several lifecycle/storage/cost gates did not pass, so no promotion occurred. Both graph corrections now pass local structural regressions; the fail-closed targeted runner requires exact replacement-image hashes before it can collect the scarf and 1/2/4-second GPU evidence. No live rerun occurred because the terminated Pod's model storage was non-persistent and the provider inventory contained no reusable volume. See [LIVE_GPU_QUALIFICATION_2026-08-26.md](LIVE_GPU_QUALIFICATION_2026-08-26.md) and [LIVE_GPU_QUALIFICATION_2026-08-26-TARGETED-FIX.md](LIVE_GPU_QUALIFICATION_2026-08-26-TARGETED-FIX.md).
...
| Workflow-registry/config/worker tests | Prove hash-locked control-guided Qwen/LTX graphs, final-I2V `8n+1` generation rounding plus strict requested-duration slicing for 1/2/4-second cases at 12 fps, exact model-free foley and official LTX trainer contracts, adaptation sample hash checks, runtime/source pins, core-image exclusion of the trainer, core-only promotion, and non-billable advanced locks | Duration checks are structural until the targeted GPU run probes the three produced files; no model/GPU output-quality proof, and all AT-043/AT-045/AT-047–AT-049 external gates remain |
| Candidate model-license and promotion-policy checks | Prove one exact-revision record for every pinned repository, policy-derived core/advanced scope coverage, the LTX-to-Gemma transitive link, named/dated/explained accepted decisions, safe pending decisions, null model hashes, unchanged manifest locks, and exclusion of LatentSync/its VAE/lip fixtures from core bundles and production promotion | The recorded individual LTX acceptance does not clarify LatentSync's binding weight license, supply voice/likeness rights, create model hashes, or authorize production |
| Worker-signing workflow tests | Four static policy tests prove manual-only/main-only dispatch, protected-environment binding, least-privilege permissions, fixed GHCR package, commit-pinned actions/Cosign, strict manifest/config digest checks, exact GitHub OIDC identity/issuer, and post-sign verification | Local structure alone cannot prove repository environment protection or a successful GitHub-hosted OIDC signing run; external activation evidence is retained in run `32967547472` and canonical referrer `sha256:c979edccb17c97a217b015429d9e39b831259389b5fdf3a69b9dc7c81e81b094` |
...
- **AT-020:** Produce draft and final I2V, A2V, keyframe, retake, and lip-dub benchmark outputs with complete manifests. For final I2V, request 1.0, 2.0, and 4.0 seconds and prove each probed output differs from its requested duration by no more than one frame at the selected frame rate; the draft fixture remains byte/hash-identical when its graph is outside the fix.
