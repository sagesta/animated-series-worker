...
| Project-store integration tests | Seven passing tests covering series+film create/reopen, verified full backup/restore with byte fixtures, no-overwrite recovery, damaged-backup refusal, canonical files/databases/folders, same-title isolation, path-like identity rejection, and live/stale/incomplete writer locks | Automated AT-001 passes locally; clean-machine AT-030, failure injection AT-004, and full AT-031 isolation remain |
| Renderer tests | Five passing tests covering honest no-paid-service empty state, four-step one-off-film creation plus the plain-language verified-backup action, Settings recovery of a missing project, local redacted-support-file creation, and the no-cost RunPod connection flow remaining generation-locked | Does not replace representative non-technical usability acceptance, packaged secret scan, clean-machine recovery drill, or live-provider test |
...
The suite contains 31 automated tests. Automated AT-001 now passes for a series and film on the development machine; the clean-machine AT-030 drill and broader interruption/migration AT-004 evidence remain. AT-013 remains open because no provider resource, worker, job, purge, watchdog, termination, or charge has been tested. AT-031 remains open because asset/query/token/cache and concurrent-worker isolation do not exist yet.
