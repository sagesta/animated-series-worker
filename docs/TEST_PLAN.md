...
| Candidate model-license evidence check | Proves one exact-revision record for every pinned repository, workflow-derived core/advanced scope coverage, the LTX-to-Gemma transitive link, pending unnamed/undated decisions, null model hashes, and unchanged `licenseReview: required` locks | Technical evidence only; it cannot accept legal/gated terms, determine entity revenue, clarify LatentSync's binding weight license, supply voice/likeness rights, or authorize production |
